...
def calculate_ambiguity():
    ...
    def ambiguity_dataset(examples):
        position = examples["position"]
        all_tables = examples["table_data"]
        all_headers = examples["table_header"]
        all_questions = examples['question']
        all_answers = examples['answer_coordinates']
            
        questions, answers, tables, headers = [], [], [], []
        ...
        def _ambiguity(answers, tables, questions, headers):
            list_amb = []
            
            logs = []

            for answer, table, q, h in zip(answers, tables, questions, headers):
                ans_times = []
                answer_text = []
                for r, c in zip(answer['row_index'], answer['column_index']):
                    times = 0
                    a = table[r][c]
                    for line in table:
                        times += 1 if a in line else 0
                    ans_times.append(times)  
                    answer_text.append(f'{h[c]} [ {a} ]')

                
                if 1 not in ans_times:
                    log = {'table': table, 'header': h,  'answer': answer_text, 'question': q}
                    logs.append(log)

                list_amb.append(1 if 1 not in ans_times else 0)  
            
            print(sum(list_amb)/len(tables))
            print(len(tables))
            return logs

        return _ambiguity(answers, tables, questions, headers)
    
    dataset = load_dataset('msr_sqa')
    
    logs = ambiguity_dataset(dataset["test"]) #+ ambiguity_dataset(dataset["train"])
    import json
    with open(f'ambiguity.json', 'w') as f:
        json.dump(logs, f, indent=4) 
    

# extract answer from: text [answer]
