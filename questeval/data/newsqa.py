...
class WebNLGDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(name="newsqa", version=VERSION, description="NewsQA datasets"),
    ]
    ...
    def _generate_examples(self, filepath, split):
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for key, raw in enumerate(data):
                context = raw["story_text"].split()
                answers = []

                for ind in raw["answer_token_ranges"].split(','):
                    s = ind.split(':')[0]
                    e = ind.split(':')[1]
                    for i in range(int(s), int(e)):
                        answers.append(context[i])

                answers.append(".")

                # Yields examples as (key, example) tuples
                yield key, {
                    "context": raw["story_text"],
                    "answer": ' '.join(answers),
