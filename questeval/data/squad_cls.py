...
class SQuADCLSDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(name="squad_cls", version=VERSION, description="Squad datasets used for inverse question answering system"),
    ]
    ...
    def _generate_examples(self, filepath, split):
        ...
        dataset = data['data'][:int(0.5*len(data['data']))]
        key = 0
        for d in dataset:
            ...
            for p in paras:           
                for qa_pair in p['qas']:        
                    ...
                    import random
                    if len(corpus) > 10:
                        corpus_selected = random.sample(corpus, 10)
                    else:
