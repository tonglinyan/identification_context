...
class QuestEval:
    ...
    def _load_all_models(self) -> Dict:
        ...
        models = {"hyp": {}}
        if self.language == 'en':
            models['hyp']['QA'] = '/home/tonglin.yan/questeval/questeval/t5_qa_squad2neg_en'
            models['hyp']['QG'] = '/home/tonglin.yan/questeval/questeval/t5_qg_squad1_en'
        else:
        ...
        if self.task == "data2text":
            models['src'] = dict()
            models['src']['QA'] = '/home/tonglin.yan/questeval/questeval/t5_qa_webnlg_en'
            models['src']['QG'] = '/home/tonglin.yan/questeval/questeval/t5_qg_webnlg_en'
    ...
    def corpus_questeval(
        ...
        if self.task == "data2text":
            having_sources = (
                sources is not None
                and all([isinstance(r, str) for rs in sources for r in rs]) 
            )
    ...
    def _batch_questeval(
        ...
        if sources is not None:
            ...
            # asking the questions from hypothesis on text from source, answer stocked in hypothesis
            modified_logs = max(self._compute_question_answering(src_logs, hyp_logs, 'src', 'hyp'), modified_logs)
            # asking the questions from source on text from hypothesis, answer stocked in source
        ...
        if list_references is not None:
            ...
            for i_ref in range(len_refs[0]):
                ...
                # Asking the questions on the compared text
                # asking the questions from hypothesis on text from reference, answer stocked in hypothesis
                modified_logs = max(self._compute_question_answering(ref_logs, hyp_logs, 'ref', 'hyp'), modified_logs)
                # asking the questions from reference on text from hypothesis, answer stocked in reference
    ...
    # text can be hypothesis, source, reference
    def _texts2logs(
        self,
        texts: List[str],
        type_logs: str,
    ...
    # text (hypothesis, source, reference)
    def _load_logs(
        self,
        texts: List,
        type_logs: str,
        ...
        for text in texts:
            log_hash = text2hash(text) # hashcode of a text
        ...
        if len(to_do_exs) != 0:
            ...
            for i in range(len(to_do_exs)):
                ...
                if name_model_qa not in logs_1[idx]['asked'][question]:
                    logs_1[idx]['asked'][question][name_model_qa] = {'answer': qa_texts[i] if qa_texts[i] != '' else 'Unanswerable',
