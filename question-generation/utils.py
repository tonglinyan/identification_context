...
def _load_webnlg()-> Dict:
    ...
    for type in ['train', 'dev', 'test']:
        for data in raw_datasets[type]:
            text = data['lex']['text']
            triple = data['modified_triple_sets']['mtriple_set'] * len(text)
            
            assert len(text) == len(triple)
...
def _load_dataset(
    ...
    for type in ['train', 'validation']:
        for idx, context in enumerate(raw_datasets[type][context_column]):
            if context not in contexts:
                log = dict()
                log['id'] = f'{dataset_name}{idx}'
                log["type"] = task
                log["context"] = context

                paragraphs = context.split('. ')
                log['sentences'] = [{"st": para} for para in paragraphs]

                for i in range(len(paragraphs)): 
                    log['sentences'][i]["id"] = f'{dataset_name}{idx}st{i}'
